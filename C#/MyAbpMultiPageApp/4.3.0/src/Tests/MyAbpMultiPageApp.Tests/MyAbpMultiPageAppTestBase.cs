...
using MyAbpMultiPageApp.Users;
using Castle.MicroKernel.Registration;
using Effort;
using EntityFramework.DynamicFilters;

namespace MyAbpMultiPageApp.Tests
{
    public abstract class MyAbpMultiPageAppTestBase : AbpIntegratedTestBase<MyAbpMultiPageAppTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MyAbpMultiPageAppTestBase()
        {
        ...
        protected void UsingDbContext(Action<MyAbpMultiPageAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyAbpMultiPageAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyAbpMultiPageAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyAbpMultiPageAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyAbpMultiPageAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyAbpMultiPageAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyAbpMultiPageAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyAbpMultiPageAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyAbpMultiPageAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyAbpMultiPageAppDbContext, Task<T>> func)
        {
            T result;
